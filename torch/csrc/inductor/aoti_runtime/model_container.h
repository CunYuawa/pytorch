 ...
 public:
  ...
  void run_const_fold(
    ...
    if (const_folded == ConstantState::FOLDED) {
      std::cerr << "[WARNING] run_const_fold: buffer is already folded for the"
                << " current weights; discarding this call. Supply new"
                << " constants via update_constant_buffer if a re-fold is"
                << " intended.\n";
      return;
    }
    if (!inactive_buffer) {
    ...
    for (size_t idx = 0; idx < num_constants; idx++) {
      ...
      if (fold_input_only) {
        ...
        if (src_device_type == const_device_type) {
          ...
          // top of this function rejects every other mismatch. Reuse the
          // load-time path: it allocates owned storage on the constant's
          // device, laid out from the constant's own shape/stride, and does
          // the throttled H2D copy.
          void* src_ptr = nullptr;
          AOTI_TORCH_ERROR_CODE_CHECK(
              aoti_torch_get_data_ptr(tensor, &src_ptr));
          models_[0]->create_owned_constant(
              idx, static_cast<const uint8_t*>(src_ptr), &materialized);
        }
        target.map->insert_or_assign(
